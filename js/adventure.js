...
function try_click(e) {
    if (adventure_is_allowed) {
        e.preventDefault();
        document.elementFromPoint(player.get_click_position_x(), player.get_click_position_y()).click();
    }
}
...
function checkForCollisions() {
    ...
    if (!player.is_rising) {
        ...
        document.getElementById("page_text").querySelectorAll("p, h1, span, button, a, tr").forEach(function (text_element) {
...
function y_collision_between(top_element, bottom_element) {
...
function updateEntities() {
    ...
    var coins = document.getElementsByClassName("coin");
    for (var i = 0; i < coins.length; i++) {
        coins[i].src = "../img/coins/coins_" + (stopwatch % 6 + 1) + ".png";
    }
}
